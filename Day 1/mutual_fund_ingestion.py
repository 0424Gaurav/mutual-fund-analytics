...
def get_scheme_code_column(df: pd.DataFrame) -> str | None:
...
def explore_fund_master(df: pd.DataFrame):
    ...
    code_column = get_scheme_code_column(df)
    if code_column is not None:
        sample_codes = df[code_column].dropna().astype(str).head(10).tolist()
        print("Sample AMFI scheme codes:", sample_codes)
        print("AMFI scheme code lengths:", sorted(df[code_column].dropna().astype(str).map(len).unique()))
    else:
        print("No scheme code column found in fund_master. Expected 'scheme_code' or 'amfi_code'.")


def validate_amfi_codes(master_df: pd.DataFrame, nav_history_df: pd.DataFrame):
    master_code_column = get_scheme_code_column(master_df)
    nav_code_column = get_scheme_code_column(nav_history_df)

    if master_code_column is None:
        print("fund_master is missing scheme_code/amfi_code column.")
        return
    if nav_code_column is None:
        print("nav_history is missing scheme_code/amfi_code column.")
        return

    master_codes = set(master_df[master_code_column].dropna().astype(str).unique())
    nav_codes = set(nav_history_df[nav_code_column].dropna().astype(str).unique())
